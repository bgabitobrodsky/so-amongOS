...
void test_tabla_segmentos(){
	...
	list_add(tablas,crear_indice(1, (void*) tabla1));
	...
	list_add(tablas,crear_indice(2, (void*) tabla2));
...
void print_tablas_segmentos_info(){
	int size = list_size(tablas);
	printf("<----- TABLAS DE SEGMENTOS ---------------------\n");
	for(int i = 0; i < size; i++) {
        indice_tabla* index = list_get(tablas, i);
